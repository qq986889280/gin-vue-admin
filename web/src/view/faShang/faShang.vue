  ...
  <div>
    <div class="gva-search-box">
      <el-form :inline="true" :model="searchInfo" class="demo-form-inline">
        ...
        <el-form-item label="区块链" prop="net">
          <el-select v-model="searchInfo.net" clearable placeholder="请选择" @clear="()=>{searchInfo.net=undefined}">
    ...
    <el-dialog v-model="dialogFormVisible" :before-close="closeDialog" title="弹窗操作">
      <el-form ref="elFormRef" :model="formData" label-position="right" :rules="rule" label-width="80px">
        ...
        <el-form-item label="合约地址:" prop="contractaddr">
          <el-input v-model="formData.contractaddr" :clearable="true" placeholder="请输入" />
        </el-form-item>
...
const getTableData = async() => {
  const table = await getFaShangList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const setOptions = async() => {
  netOptions.value = await getDictFunc('net')
}
...
const onDelete = async() => {
  ...
  const res = await deleteFaShangByIds({ ids })
  if (res.code === 0) {
    ElMessage({
      type: 'success',
      message: '删除成功'
...
// 更新行
const updateFaShangFunc = async(row) => {
  const res = await findFaShang({ ID: row.ID })
  type.value = 'update'
  if (res.code === 0) {
    formData.value = res.data.refaShang
    dialogFormVisible.value = true
  }
}

// 删除行
const deleteFaShangFunc = async(row) => {
  const res = await deleteFaShang({ ID: row.ID })
  if (res.code === 0) {
    ElMessage({
      type: 'success',
      message: '删除成功'
...
const enterDialog = async() => {
     elFormRef.value?.validate(async(valid) => {
       ...
       switch (type.value) {
         case 'create':
           res = await createFaShang(formData.value)
           break
         case 'update':
           res = await updateFaShang(formData.value)
           break
         default:
           res = await createFaShang(formData.value)
           break
